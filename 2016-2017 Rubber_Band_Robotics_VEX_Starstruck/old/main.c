...
void updateLock(){
	...
	else if(armState == "launching"){
		if(currentTime - timeStartedLaunching < 125){
			motor[release]  = -127;
		}
		else{
			motor[release] = 0;
...
void getInput()
{
	if(vexRT[Btn8D])
	{
		...
		{
			armState = "launching";
			timeStartedLaunching = currentTime;
		}
...
void manuaCheck(){
	if((vexRT[Btn5DXmtr2] && vexRT[Btn5UXmtr2]) && (vexRT[Btn6DXmtr2] && vexRT[Btn6UXmtr2])){						//Something bad has happened...
		manual = true;
	}
	else if((vexRT[Btn5D] && vexRT[Btn5U]) && (vexRT[Btn6D] && vexRT[Btn6U])){													//WAIT I DIDNT MEAN TO DO THAT
		manual = false;
	}
}
task bla(){
